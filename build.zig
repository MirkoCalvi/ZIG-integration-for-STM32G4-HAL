...
pub fn build(b: *std.Build) !void {
    ...
    exe.addIncludePath(b.path("src/boards/stm32g4xx-hal-driver/Inc"));
    //exe.addIncludePath(b.path("./src/boards/stm32g4xx-hal-driver/Src"));
    ...
    switch (board_choice_enum) {
        Board_list.stm32g4xx => {
            ...
            try addCSourcesFromDir(b, exe, "src/boards/stm32g4xx-hal-driver/Src");
        },
        else => {
            //no import is needed
        },
...
fn addCSourcesFromDir(b: *std.Build, exe: *std.Build.Step.Compile, dir_path: []const u8) !void {
    const fs = std.fs;
    ...
    while (try it.next()) |entry| {
        if (entry.kind == .file) { //and fs.path.isExtension(entry.name, "c")
            // Add each `.c` file to the executable
